...
function findOtherNodesInLayer(nodeX, layer_dims)
    """
    Takes a random node X, returns the start and end index of its previous layer
    """
    inds = [1]
    l = 0
...
function rewireOutdegree(AInit, D, pRandRewire, layer_dims)
        ...
        # rewiring is constrained b/w layers, notX is returned accordingly as nodes in preview layers
        notX, nodeX = pickNodes(A, layer_dims)

        # find the nodes with no incomings to "x"
        xNonHeadBool = 1 .* Bool.(A[notX, nodeX] .== 0) 
        
        # xNonHeadBool doesn't contain any non-heads, no rewiring 
        ...
        if pRandRewire <= rand()
            ...
            # find the coldest node 
            xCutHead = xHeads[argmin(A[xHeads, nodeX])]
            #xHeads[argmax(transpose(sum(D[xHeads,:], dims = 2))+sum(D[:,xHeads], dims=1))[2]] # also tried this, it was worse 
        ...
        # no random rewiring takes place but script is still here
