module mlp 
include("./rewiring.jl")
using Knet, Plots, Statistics, LinearAlgebra, Random,.rewiring
export pred, loss, gen_and_split_data, genbatch, backprop
...
function gen_and_split_data(n,ntest)
...
function genbatch(nbit,nbat, ntest)
    testidx = randperm(2^nbit-1)[1:ntest]
    trainidx = setdiff(1:2^nbit-1,testidx) # samples not in the test set
...
function backprop(w, b, niter, n, ntest, nbatch, layer_dims; rewire = false, λ= 0.0, LR = 0.1, p = 0.0)
    losstrn = []
    losstst = []
    xtrn, ytrn, xtst, ytst = gen_and_split_data(n, ntest)
    
    for k in progress(1:niter)
        xin = genbatch(n,nbatch, ntest)
        yin = sum(xin,dims=1).%2
        dl = @diff loss(w,b,xin,yin; λ= 0.001)
        for i=1:length(w)
            w[i] .-= LR *grad(dl,w[i])
            b[i] .-= LR *grad(dl,b[i])
        end
        ...
        if p >= rand()
            ...
            ############### PRUNE OUT-DEGREES ###############
            A, o, xCutHead, nodeX = pruneOutDegrees(A, p, LR)
            ############### REWIRE OUT-DEGREES ##############
            if rewire == true
                A = rewireOutDegree(A, o, xCutHead, nodeX, 0.3)
            end
            #################################################
        ...
        if (k%1000==1)
            xin = genbatch(n,ntest, ntest) # training set samples with size equal to test set
            yin = sum(xin,dims=1).%2 
            push!(losstrn,loss(w,b,xin,yin; λ= 0.001)) # record loss over 1000 samples
            push!(losstst,loss(w,b,xtst,ytst; λ= 0.001))
        end
    end
    
    return w, b, losstrn, losstst
end
end